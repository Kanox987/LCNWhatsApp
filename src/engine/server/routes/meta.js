...
export function registrarRotasMeta (roteador, db) {
  ...
  roteador.get('/meta/variables', () => ({
    builtIn: [
      ...
      { namespace: 'sender', field: 'isAdmin', example: '{{sender.isAdmin}}', description: 'Diz se quem enviou é administrador DESTE grupo (true) ou não (false). É o que permite montar \"só admin pode usar este comando\", comparando na condição.', warning: { level: 'compatibility', text: 'Só existe em grupo, e só depois de o bot conseguir consultar os dados dele. Quando não vier, o texto aparece do jeito que está e NUNCA vira \"false\": \"não consegui saber\" e \"não é admin\" são coisas diferentes, e tratar uma como a outra faria falha de rede virar decisão de permissão.' } },
      ...
      { namespace: 'chat', field: 'name', example: '{{chat.name}}', description: 'Nome do grupo. Serve para dar boas-vindas dizendo onde a pessoa entrou.', warning: { level: 'compatibility', text: 'Só existe em grupo, e só depois de o bot conseguir consultar os dados dele. Em conversa privada, ou enquanto a consulta não responde, o texto aparece do jeito que está.' } },
      { namespace: 'chat', field: 'size', example: '{{chat.size}}', description: 'Quantas pessoas há no grupo agora.', warning: { level: 'compatibility', text: 'Só existe em grupo, e só depois de o bot conseguir consultar os dados dele. Em conversa privada, ou enquanto a consulta não responde, o texto aparece do jeito que está.' } },
      { namespace: 'chat', field: 'onlyAdmins', example: '{{chat.onlyAdmins}}', description: 'true quando o grupo está configurado para só administradores escreverem.', warning: { level: 'compatibility', text: 'Só existe em grupo, e só depois de o bot conseguir consultar os dados dele. Em conversa privada, ou enquanto a consulta não responde, o texto aparece do jeito que está.' } },
      { namespace: 'bot', field: 'id', example: '{{bot.id}}', description: 'O número que está executando a automação. Serve para o bot se apresentar sem você digitar o próprio número dentro da resposta.', warning: { level: 'compatibility', text: 'Depende de a sessão já ter informado o número da conta. Enquanto não informar, o texto fica do jeito que está em vez de sair um identificador interno no lugar de um telefone.' } },
      { namespace: 'bot', field: 'isAdmin', example: '{{bot.isAdmin}}', description: 'Diz se o PRÓPRIO bot é administrador deste grupo. Apagar mensagem e remover participante só funcionam quando é. Vale checar antes e avisar, em vez de tentar e falhar na frente de todo mundo.', warning: { level: 'compatibility', text: 'Só existe em grupo, e só depois de o bot conseguir consultar os dados dele. Quando não vier, o texto aparece do jeito que está e NUNCA vira \"false\": \"não consegui saber\" e \"não é admin\" são coisas diferentes, e tratar uma como a outra faria falha de rede virar decisão de permissão.' } },
