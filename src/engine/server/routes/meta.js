...
export function registrarRotasMeta (roteador, db) {
  ...
  roteador.get('/meta/variables', () => ({
    builtIn: [
      ...
      { namespace: 'sender', field: 'name', example: '{{sender.name}}', description: 'O nome que a pessoa escolheu exibir no WhatsApp. Serve para cumprimentar pelo nome em vez de pelo número. O sistema lembra o nome de quem já falou, então vale mesmo nas mensagens em que o WhatsApp não manda o nome junto.' },
      { namespace: 'sender', field: 'isAdmin', example: '{{sender.isAdmin}}', description: 'Diz se quem enviou é administrador DESTE grupo (true) ou não (false). É o que permite montar \"só admin pode usar este comando\", comparando na condição. Existe só em conversa de grupo.' },
      ...
      { namespace: 'chat', field: 'name', example: '{{chat.name}}', description: 'Nome do grupo. Serve para dar boas-vindas dizendo onde a pessoa entrou. Existe só em conversa de grupo.' },
      { namespace: 'chat', field: 'size', example: '{{chat.size}}', description: 'Quantas pessoas há no grupo agora. Existe só em conversa de grupo.' },
      { namespace: 'chat', field: 'onlyAdmins', example: '{{chat.onlyAdmins}}', description: 'true quando o grupo está configurado para só administradores escreverem. Existe só em conversa de grupo.' },
      ...
      { namespace: 'bot', field: 'isAdmin', example: '{{bot.isAdmin}}', description: 'Diz se o PRÓPRIO bot é administrador deste grupo. Apagar mensagem e remover participante só funcionam quando é. Vale checar antes e avisar, em vez de tentar e falhar na frente de todo mundo. Existe só em conversa de grupo.' },
